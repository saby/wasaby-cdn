...
var OS = /** @class */ (function () {
    ...
    OS.detect = function (userAgent_) {
        ...
        if (!familyName) {
            console.warn(
                '\u041D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u043E\u043F\u0440\u0435\u0434\u0435\u043B\u0438\u0442\u044C \u041E\u0421 \u0434\u043B\u044F \u0437\u043D\u0430\u0447\u0435\u043D\u0438\u044F UserAgent ' +
                    userAgent +
                    ' :: ' +
                    typeof userAgent
            );
        ...
        var version =
            parseFloat((family.version.exec(userAgent) || [null]).pop()) ||
            family.available;
        return {
...
var Browser = (function () {
    ...
    Browser.detect = function (userAgent_) {
        ...
        if (!familyName) {
            console.warn(
                '\u041D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u043E\u043F\u0440\u0435\u0434\u0435\u043B\u0438\u0442\u044C \u0431\u0440\u0430\u0443\u0437\u0435\u0440 \u0434\u043B\u044F \u0437\u043D\u0430\u0447\u0435\u043D\u0438\u044F UserAgent ' +
                    userAgent +
                    ' :: ' +
                    typeof userAgent
            );
        ...
        var _a = family.version,
            reg = _a.reg,
            available = _a.available,
            availableForIESaby = _a.availableForIESaby;
        var version =
            parseFloat((reg.exec(userAgent) || [null]).pop()) || available; // при неопредленной версии считаем браузер актуальным
        return {
            isAvailable:
                version >= (!isDomenForIE() ? available : availableForIESaby),
            isCompatibleMode:
                isCompatibleMode(userAgent) ||
                (document.documentMode && version !== document.documentMode),
            name: family.name,
            version: version,
            family: family,
        };
    ...
    function isCompatibleMode(ua) {
        if (!ua || !/Trident/i.test(ua)) {
            return false;
        }
        var trident = parseFloat(
            (/Trident\/([0-9]{1,}[.0-9]{0,})/i.exec(ua) || [0]).pop()
        );
        var ie = parseFloat(
            (/(MSIE |rv:)([0-9]{1,}[.0-9]{0,})/i.exec(ua) || [0]).pop()
        );
        /**
...
function isSoftwareAvailable(userAgent) {
    ...
    return browser.isAvailable && os.isAvailable;
}

function isDomenForIE() {
    var domains = location.hostname.split('.');
    var isAutoTest = location.hostname.indexOf('autotest') !== -1;
    // для адреса *ie.saby.ru/reg/ используем обычную проверку.
    var isReg = regCheck();
    return (
        !isReg &&
        (isAutoTest ||
            (domains.length === 3 &&
                domains[0].split('-').some(function (search) {
                    return search == 'ie';
                })))
    );
}

function allowWorkInIE(userAgent) {
    var browser = Browser.detect(userAgent);
    return browser.isAvailable && isDomenForIE();
}

function regCheck() {
    return location.pathname.indexOf('/reg/') === 0;
}

(function () {
    // если это офлайн  - не проверяем
    if (document.cookie.indexOf('desktop_app=true') !== -1) {
        return;
    }
    // если это реклама на старых приставках  - не проверяем
    if (document.cookie.indexOf('isWebView=true') !== -1) {
        return;
    }
    ...
    // мы не на домене *ie
    if (!isDomenForIE()) {
        // обычная проверка на доступность
        if (!isSoftwareAvailable(window.navigator.userAgent)) {
            redirect();
        }
    } else {
        if (!allowWorkInIE(window.navigator.userAgent)) {
            redirect();
        }
    }
    ...
    function redirect() {
        ...
        var DIAGNOSTIC_PAGE = '/cdn/Maintenance/1.0.40/html/diagnostic.html';
    ...
    function redirectToIESaby() {
        var returnLink = window.location.href.split(window.location.host).pop();

        returnLink = returnLink
            .replace('/auth-online/', '/auth/')
            .replace(/\?ret=(.*)/, '');
        var preDomens = location.host.split('.');
        var preDomen = preDomens[0];
        var parts = preDomen.split('-');
        var index = 0;
        var result;
        var testStand = parts.filter(function (x, _index) {
            var isFinded = ['test', 'dev', 'fix'].indexOf(x) !== -1;
            if (isFinded) {
                index = _index;
            }
            return isFinded;
        ...
        // если это не сайты *-1с,*-sap , то отбарысываем домен третьго уровня
        if (['sap', '1c'].indexOf(parts[parts.length - 1]) === -1) {
            parts = parts.slice(0, -1);
        }

        // Добавляем новыё уровень третьего домена ie
        if (testStand.length) {
            testStand[0] += '-' + target;
            parts[index] = testStand[0];
            result = parts.join('-');
        } else {
            result = [target].concat(parts).join('-');
        }

        preDomens[0] = result;
        preDomens[1] = 'saby';
        var targetLocation = preDomens.join('.');

        var newHref =
            window.location.protocol + '//' + targetLocation + returnLink;
    ...
    function isMobileSafariDetect(userAgent) {
        ...
        return (
            IOS_REGEXP.test(userAgent) ||
            (MAC_OS_REGEXP.test(userAgent) &&
                navigator &&
