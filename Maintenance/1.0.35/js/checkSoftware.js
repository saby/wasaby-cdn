...
var Browser = (function () {
   ...
   Browser.detect = function (userAgent_) {
      ...
      var _a = family.version, reg = _a.reg, available = _a.available, availableForIESaby = _a.availableForIESaby;
      var version = parseFloat((reg.exec(userAgent) || [null]).pop()) || available; // при неопредленной версии считаем браузер актуальным
      return {
         isAvailable: version >= (!isDomenForIE() ? available : availableForIESaby),
...
function isSoftwareAvailable(userAgent) {
   ...
   return browser.isAvailable
      ...
      browser.family === Browser.family.YaBrowser && isOldWindows(userAgent);
}
...
function isDomenForIE() {
   ...
   var isReg = location.pathname.indexOf('/reg/') === 0;
   return !isReg && (isAutoTest || (domains.length === 3 && domains[0].split('-').some(function (search) {
      return search == 'ie';
   })));
}

function isOldWindows(userAgent) {
   var os = OS.detect(userAgent);

   return os.version >= OS.list.WindowsXP.version
      && os.version <= OS.list.WindowsVista.version;
}

function chechYandexForOldWindows(userAgent) {
   var browser = Browser.detect(userAgent);
   return browser.family === Browser.family.YaBrowser && isOldWindows(userAgent);
}
...
(function () {
   ...
   // мы не на домене *ie
   if (!isDomenForIE()) {
      // Если это XP/VISTA+YaBrowser -> перенаправляем на *ie
      if (chechYandexForOldWindows(window.navigator.userAgent)) {
         redirectToIESaby();
         return;
      }
   ...
   function redirect() {
      ...
      var DIAGNOSTIC_PAGE = '/cdn/Maintenance/1.0.35/html/diagnostic.html';
   ...
   function redirectToIESaby() {
      ...
      returnLink = returnLink.replace('/auth-online/', '/auth/').replace(/\?ret=(.*)/, '');
      ...
      window.location.href = window.location.protocol + '//' + targetLocation
         + returnLink;
   }
