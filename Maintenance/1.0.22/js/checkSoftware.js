...
function isSoftwareAvailable(userAgent) {
   ...
   if (allowWorkInIE(browser, userAgent) && browser.isAvailable) {
      return true;
   }
...
function allowWorkInIE(browser) {
   var domains = location.host.split('.');
   domains.length === 3;
   return browser.family === Browser.family.IE && (domains.length === 3 && domains[0].split('-').some(function (search) {
      return search == 'ie';
   }));
}
...
(function () {
   ...
   function redirect() {
      ...
      var DIAGNOSTIC_PAGE = '/cdn/Maintenance/1.0.22/html/diagnostic.html';
