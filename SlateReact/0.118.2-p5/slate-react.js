...
(function (global, factory) {
  if (typeof module === "object" && typeof module.exports === "object") {
    var req = global && global.requirejs ? global.requirejs : require;
    factory(
      module.exports,
      req("react"),
      req("react-dom"),
      req("/cdn/Slate/0.118.1-p1/slate.min.js"),
      req("Env/Env"),
    );
    ...
    define([
      ...
      "react",
      "react-dom",
      "/cdn/Slate/0.118.1-p1/slate.min.js",
      "Env/Env",
    ], factory);
  }
})(
  ...
  function (exports, React, ReactDOM, slate, env) {
      ...
      r = (e, r) => {
        ...
        for (let t = 0; t < w.length; t++) {
          ...
          let T = 0,
            B = 0;
          const F =
              "offsetWidth" in e ? e.offsetWidth - e.clientWidth - a - p : 0,
            V =
              "offsetHeight" in e ? e.offsetHeight - e.clientHeight - g - W : 0,
            S =
              "offsetWidth" in e
                ? 0 === e.offsetWidth
                  ? 0
          ...
          if (m === e)
            ...
            ((T =
              0 === X
                ? 0
                : Math.max(0, Math.min(h + T / X, e.scrollHeight - n / X + V))),
              (B =
                0 === S
                  ? 0
                  : Math.max(
                      0,
                      Math.min(t + B / S, e.scrollWidth - l / S + F),
                    )),
              (k += h - T),
              (D += t - B));
          }
    ...
    var Path = {
      ...
      isAncestor(path, another) {
        return (
          path.length < another.length && Path.compare(path, another) === 0
        );
      },
      ...
      parent(path) {
        if (path.length === 0) {
          throw new Error(
            "Cannot get the parent path of the root path [".concat(path, "]."),
          );
        }
    ...
    var Node = {
      ...
      matches(node, props) {
        return (
          ...
          (Node.isText(node) &&
            Text$1.isTextProps(props) &&
            Text$1.matches(node, props))
        );
      },
      nodes(root) {
        var options =
    ...
    var Location = {
      isLocation(value) {
        return (
          Path.isPath(value) || Point.isPoint(value) || Range.isRange(value)
        );
      },
      isPath(at) {
     ...
     */
    var NON_SETTABLE_SELECTION_PROPERTIES = Object.getOwnPropertyNames(
      Object.prototype,
    );
    // eslint-disable-next-line no-redeclare
            ...
            modifyDescendant(editor, _path6, (node) => {
              ...
              for (var _key in newProperties) {
                if (NON_SETTABLE_NODE_PROPERTIES.includes(_key)) {
                  throw new Error(
                    'Cannot set the "'.concat(_key, '" property of nodes!'),
                  );
                }
            ...
            modifyChildren(editor, Path.parent(_path7), (children) => {
              ...
              for (var _key4 in _properties) {
                if (NON_SETTABLE_NODE_PROPERTIES.includes(_key4)) {
                  throw new Error(
                    'Cannot set the "'.concat(_key4, '" property of nodes!'),
                  );
                }
    ...
    var Transforms = _objectSpread$9$1(
      _objectSpread$9$1(
        _objectSpread$9$1(
          _objectSpread$9$1({}, GeneralTransforms),
          NodeTransforms,
        ),
        SelectionTransforms,
      ),
    ...
    var getDefaultView = (value) => {
      return (
        (value && value.ownerDocument && value.ownerDocument.defaultView) ||
        null
      );
    };
    /**
    ...
    typeof navigator !== "undefined" &&
      ...
      ((_navigator$userAgent$ = navigator.userAgent.match(/Version\/(\d+)/)) !==
        null &&
      _navigator$userAgent$ !== void 0 &&
      _navigator$userAgent$[1]
        ? parseInt(
            (_navigator$userAgent$2 =
              navigator.userAgent.match(/Version\/(\d+)/)) === null ||
    ...
    // ПРОПАТЧЕНО: http://online.saby.ru/opendoc.html?guid=f9aff325-64c6-486b-bb30-6afc29c7a4f8&client=3
    // Отслеживает единственный "живой" отложенный ретрай ReactEditor.focus() на editor.
    // Нужно, чтобы при повторном/параллельном вызове focus() или при реальном
    // размонтировании Editable не копились "осиротевшие" setTimeout-и, каждый из
    // которых бесполезно проверяет уже неактуальный editor.
    var EDITOR_TO_PENDING_FOCUS_RETRY = new WeakMap();
    var cancelPendingFocusRetry = function cancelPendingFocusRetry(editor) {
      var token = EDITOR_TO_PENDING_FOCUS_RETRY.get(editor);
      if (token) {
        token.cancelled = true;
        EDITOR_TO_PENDING_FOCUS_RETRY.delete(editor);
      }
    };
    /**
    ...
    var DOMEditor = {
      ...
      androidScheduleFlush: (editor) => {
        ...
        (_EDITOR_TO_SCHEDULE_F = EDITOR_TO_SCHEDULE_FLUSH.get(editor)) ===
          null ||
      ...
      focus: function focus(editor) {
        ...
        // ПРОПАТЧЕНО: http://online.saby.ru/opendoc.html?guid=f9aff325-64c6-486b-bb30-6afc29c7a4f8&client=3
        // Свежий вызов focus() отменяет собственный предыдущий отложенный ретрай
        // этого же editor — иначе при частых повторных вызовах (например, при
        // серии unmount/mount виджетов) в очереди накапливаются параллельные
        // цепочки setTimeout, каждая из которых бесполезно досчитывает свои
        // 5 попыток по 10мс уже после того, как её результат никому не нужен.
        cancelPendingFocusRetry(editor);
        ...
        if (editor.operations.length > 0) {
          // ПРОПАТЧЕНО: http://online.saby.ru/opendoc.html?guid=f9aff325-64c6-486b-bb30-6afc29c7a4f8&client=3
          // Раньше здесь стоял setTimeout(..., 10) — фиксированный макротаск,
          // который под частыми последовательными вызовами focus() (например,
          // при быстром удержании Backspace, когда виджеты часто переключаются)
          // конкурирует за очередь макротасков с остальной работой браузера и
          // может исполниться с задержкой в секунды вместо 10мс. operations
          // сбрасывается через собственный Promise.resolve().then() внутри
          // Editor.apply (slate.js), поэтому достаточно подождать один тик
          // микротасков — это и быстрее, и не создаёт отдельной очереди задач.
          var token = { cancelled: false };
          EDITOR_TO_PENDING_FOCUS_RETRY.set(editor, token);
          queueMicrotask(() => {
            if (token.cancelled) {
              return;
            }
        ...
        if (root.activeElement !== el) {
          ...
          if (editor.selection && root instanceof Document) {
            ...
            try {
              ...
              // ПРОПАТЧЕНО: http://online.saby.ru/opendoc.html?guid=f9aff325-64c6-486b-bb30-6afc29c7a4f8&client=3
              // Пустая очередь editor.operations (проверка выше) означает только то, что
              // Slate применил свои операции к модели — она ничего не гарантирует про
              // React: DOM для только что вставленных/изменённых узлов (например,
              // вставленной цитаты и параграфа после неё) коммитится React'ом асинхронно
              // и может быть ещё не готов в этот момент, тогда toDOMRange кидает
              // "Cannot resolve a DOM point from Slate point". В отличие от pending
              // operations (микротаск), для коммита React'а нужен минимум один кадр
              // отрисовки, поэтому ждём requestAnimationFrame, а не queueMicrotask.
              var token = { cancelled: false };
              EDITOR_TO_PENDING_FOCUS_RETRY.set(editor, token);
      ...
      toDOMNode: (editor, node) => {
        ...
        var domNode =
          node === editor
            ...
            : (_EDITOR_TO_KEY_TO_ELE = EDITOR_TO_KEY_TO_ELEMENT.get(editor)) ===
                  null || _EDITOR_TO_KEY_TO_ELE === void 0
              ? void 0
      ...
      toDOMRange: (editor, range) => {
        ...
        var domFocus = Range.isCollapsed(range)
          ? domAnchor
        ...
        var isStartAtZeroWidth = !!startEl.getAttribute(
      ...
      toSlatePoint: (editor, domPoint, options) => {
        ...
        var [nearestNode, nearestOffset] = exactMatch
          ? domPoint
        ...
        if (parentNode) {
          ...
          var editorEl = DOMEditor.toDOMNode(editor, editor);
          var potentialVoidNode = parentNode.closest(
          ...
          var voidNode =
            potentialVoidNode &&
            containsShadowAware(editorEl, potentialVoidNode)
              ? potentialVoidNode
              : null;
          var potentialNonEditableNode = parentNode.closest(
          ...
          if (leafNode) {
            ...
            var getLeafNodes = (node) =>
              node
                ...
                : [];
            var elementNode = nonEditableNode.closest(
        ...
        if (IS_ANDROID && !textNode && !exactMatch) {
          ...
          if (
            ...
            var { path: _path, offset: _offset } = Editor.start(
              editor,
              nodePath,
            );
        ...
        if (!textNode) {
          ...
          throw new Error(
            "Cannot resolve a Slate point from DOM point: ".concat(domPoint),
          );
        }
      ...
      toSlateRange: (editor, domRange, options) => {
        ...
        var { exactMatch, suppressThrow } = options;
        var el = isDOMSelection(domRange)
          ? domRange.anchorNode
          ...
          if (isDOMSelection(domRange)) {
            ...
            if (IS_FIREFOX && domRange.rangeCount > 1) {
              ...
              if (
                ...
                var firstNode = getLastChildren(
                  firstNodeRow.children[firstRange.startOffset],
                );
                var lastNode = getLastChildren(
        ...
        if (
          ...
          throw new Error(
            "Cannot resolve a Slate range from DOM range: ".concat(domRange),
          );
        }
      ...
      e.deleteBackward = (unit) => {
        ...
        if (e.selection && Range.isCollapsed(e.selection)) {
          ...
          if (parentBlockEntry) {
            ...
            var parentElementRange = Editor.range(
              e,
              parentBlockPath,
              e.selection.anchor,
            );
      ...
      e.apply = (op) => {
        ...
        if (
          ...
          var at = Location.isPoint(
            pendingAction === null || pendingAction === void 0
              ? void 0
              : pendingAction.at,
          )
        ...
        switch (op.type) {
          ...
          case "set_selection": {
            ...
            (_EDITOR_TO_USER_SELEC = EDITOR_TO_USER_SELECTION.get(e)) ===
              null ||
    ...
    function createAndroidInputManager(_ref) {
      ...
      var applyPendingSelection = function applyPendingSelection() {
        ...
        if (pendingSelection) {
          ...
          if (
            normalized &&
            (!selection || !Range.equals(normalized, selection))
          ) {
      ...
      var performAction = function performAction() {
        ...
        if (action.at) {
          ...
          if (
            !editor.selection ||
            !Range.equals(editor.selection, _targetRange)
          ) {
      ...
      var flush = function flush() {
        ...
        while (
          (diff =
            (_EDITOR_TO_PENDING_DI = EDITOR_TO_PENDING_DIFFS.get(editor)) ===
              null || _EDITOR_TO_PENDING_DI === void 0
              ? void 0
          ...
          EDITOR_TO_PENDING_DIFFS.set(
            editor,
            (_EDITOR_TO_PENDING_DI2 = EDITOR_TO_PENDING_DIFFS.get(editor)) ===
              null || _EDITOR_TO_PENDING_DI2 === void 0
              ? void 0
        ...
        var selection =
          selectionRef === null || selectionRef === void 0
            ? void 0
            : selectionRef.unref();
        if (
          selection &&
      ...
      var storeDiff = function storeDiff(path, diff) {
        ...
        var pendingDiffs =
          (_EDITOR_TO_PENDING_DI3 = EDITOR_TO_PENDING_DIFFS.get(editor)) !==
            null && _EDITOR_TO_PENDING_DI3 !== void 0
      ...
      var hasPendingDiffs = function hasPendingDiffs() {
        ...
        return !!(
          (_EDITOR_TO_PENDING_DI4 = EDITOR_TO_PENDING_DIFFS.get(editor)) !==
            null &&
      ...
      var handleDomMutations = function handleDomMutations(mutations) {
        ...
        if (
          ...
          (_EDITOR_TO_FORCE_REND = EDITOR_TO_FORCE_RENDER.get(editor)) ===
            null ||
    ...
    var isElement = function (obj) {
      ...
      var scope =
        (_a = obj === null || obj === void 0 ? void 0 : obj.ownerDocument) ===
          null || _a === void 0
          ? void 0
          : _a.defaultView;
    ...
    var IE = /msie|trident/i.test(
      global$1.navigator && global$1.navigator.userAgent,
    );
    ...
    var Scheduler = (function () {
      ...
      Scheduler.prototype.observe = function () {
        ...
        document.body
          ? cb()
          : global$1.addEventListener("DOMContentLoaded", cb);
      };
        ...
        if (this.stopped) {
          ...
          this.observer =
            typeof MutationObserver !== "undefined"
              ? new MutationObserver(this.listener)
              : null;
          this.observe();
    ...
    var ZeroWidthString = function ZeroWidthString(props) {
      var _props$length = props.length,
        ...
        isLineBreak =
          _props$isLineBreak === void 0 ? false : _props$isLineBreak,
        _props$isMarkPlacehol = props.isMarkPlaceholder,
        isMarkPlaceholder =
          _props$isMarkPlacehol === void 0 ? false : _props$isMarkPlacehol;
      var attributes = {
      ...
      // ПРОПАТЧЕНО
      // ошибка на айпаде при открытии клавиатуры буквы маленькие, а не большие
      // !IS_ANDROID -> !(IS_ANDROID || IS_IOS) -> !isMobile
      // вот тут Назар разбирался с патчем
      // https://online.sbis.ru/opendoc.html?guid=1a6464ba-86d9-460f-b1f5-f3b32320c97b&client=3

      // ПРОПАТЧЕНО
      // Проверки IS_ANDROID/IS_IOS ломаются в мобильных webview, из-за чего едет гидратация.
      // Env.detection определяет мобильную платформу корректно.
      // https://online.sbis.ru/opendoc.html?guid=019ef9e0-ded7-7ca6-9cd0-8864d45d487e&client=3
      var isMobile = env.detection.isMobileAndroid || env.detection.isMobileIOS;
      return /*#__PURE__*/ React.createElement(
        "span",
        _objectSpread$6({}, attributes),
        !isMobile || !isLineBreak ? "\uFEFF" : null,
    ...
    var Leaf = function Leaf(props) {
      var leaf = props.leaf,
        ...
        renderLeaf =
          _props$renderLeaf === void 0 ? defaultRenderLeaf : _props$renderLeaf,
            ...
            if (!placeholderResizeObserver.current) {
              ...
              placeholderResizeObserver.current = new ResizeObserver$1(
                function () {
                  ...
                  (_leaf$onPlaceholderRe2 = leaf.onPlaceholderResize) ===
                    null ||
    ...
    var useDecorations = function useDecorations(node, parentDecorations) {
      ...
      var equalityFn = Node.isText(node)
    ...
    var Text = function Text(props) {
      var parentDecorations = props.decorations,
        ...
        renderText =
          _props$renderText === void 0 ? defaultRenderText : _props$renderText,
        text = props.text;
      var editor = useSlateStatic();
      var ref = React.useRef(null);
    ...
    var MemoizedElement = /*#__PURE__*/ React.memo(
      Element$1,
      function (prev, next) {
        return (
    ...
    var ChunkTreeHelper = /*#__PURE__*/ (function () {
      ...
      _createClass(ChunkTreeHelper, [
        ...
        },
        {
          ...
          get: function get() {
            return (
              this.pointerChunk.type === "root" && this.pointerIndex === -1
            );
          },
          /**
        ...
        },
        {
          ...
          value: function enterChunk(end) {
            ...
            // istanbul ignore next
            if (
              ((_this$pointerNode = this.pointerNode) === null ||
              _this$pointerNode === void 0
                ? void 0
        ...
        },
        {
          ...
          value: function enterChunkUntilLeaf(end) {
            while (
              ((_this$pointerNode2 = this.pointerNode) === null ||
              _this$pointerNode2 === void 0
                ? void 0
        ...
        },
        {
          ...
          value: function rawInsertAfter(leaves, minDepth) {
            ...
            if (leaves.length === 0) return;
            var groupIntoChunks = function groupIntoChunks(
              leaves,
              parent,
              perChunk,
            ) {
    ...
    var reconcileChildren = function reconcileChildren(editor, _ref) {
      var chunkTree = _ref.chunkTree,
        ...
        rerenderChildren =
          _ref$rerenderChildren === void 0 ? [] : _ref$rerenderChildren,
      ...
      var _loop = function _loop() {
        ...
        var wasMoved =
          lookAhead > 0 && chunkTree.movedNodeKeys.has(treeLeaf.key);
    ...
    var ChunkTree = function ChunkTree(props) {
      ...
      return /*#__PURE__*/ React.createElement(
        ChunkAncestor,
        _objectSpread$1({}, props),
      );
    };
    var MemoizedChunk = /*#__PURE__*/ React.memo(
      ChunkAncestor,
      function (prev, next) {
        return (
          prev.root === next.root &&
    ...
    var useChildren = function useChildren(props) {
      ...
      var _useDecorationsByChil = useDecorationsByChild(
          editor,
          node,
          decorations,
        ),
    ...
    var useDecorationsByChild = function useDecorationsByChild(
      editor,
      node,
      decorations,
    ) {
      var decorationsByChild = splitDecorationsByChild(
        editor,
        node,
        decorations,
      );
    ...
    var createRestoreDomManager = function createRestoreDomManager(
      editor,
      receivedUserInput,
    ) {
    ...
    // ПРОПАТЧЕНО
    // На iPadOS зажатый Backspace шлет deleteContentBackward пачками (chunked delete),
    // из-за чего WebKit успевает физически удалить элементы из DOM раньше, чем отработает
    // контролируемый апдейт Slate. Данные в Slate остаются корректными, но верстка теряет
    // узлы, из-за чего DOM-Editor не может резолвить DOM-точки/узлы по Slate-путям, и
    // редактор ломается. RestoreDOM - механизм на MutationObserver, откатывающий такие
    // преждевременные нативные правки DOM, был доступен только на Android. Включаем и для iOS.
    var RestoreDOM =
      IS_ANDROID || IS_IOS
        ? RestoreDOMComponent
        : function (_ref) {
     ...
     * Editable.
     */
    var Editable = /*#__PURE__*/ React.forwardRef(
      function (props, forwardedRef) {
        ...
        var autoFocus = props.autoFocus,
          ...
          decorate =
            _props$decorate === void 0 ? defaultDecorate : _props$decorate,
          ...
          scrollBoundary =
            _props$scrollBoundary === void 0
              ? function () {}
              : _props$scrollBoundary,
          ...
          disableDefaultStyles =
            _props$disableDefault === void 0 ? false : _props$disableDefault,
          ...
          function () {
            return throttle_1(function () {
              ...
              if (
                ...
                var focusNodeInEditor = ReactEditor.hasTarget(
                  editor,
                  focusNode,
                );
        ...
        useIsomorphicLayoutEffect(function () {
          ...
          var setDomSelection = function setDomSelection(forceChange) {
            ...
            if (IS_FIREFOX && domSelection.rangeCount > 1) {
              ...
              var lastRange = domSelection.getRangeAt(
          ...
          var ensureSelection =
            ((_androidInputManagerR2 = androidInputManagerRef.current) ===
              null || _androidInputManagerR2 === void 0
              ? void 0
          ...
          var animationFrameId = requestAnimationFrame(function () {
            if (ensureSelection) {
              var ensureDomSelection = function ensureDomSelection(
                forceChange,
              ) {
                try {
                  var el = ReactEditor.toDOMNode(editor, editor);
            ...
            if (
              ...
              if (
                ...
                if (!IS_NODE_MAP_DIRTY.get(editor)) {
                  ...
                  var _ReactEditor$toDOMPoi = ReactEditor.toDOMPoint(
                      editor,
                      anchor,
                    ),
                    _ReactEditor$toDOMPoi2 = _slicedToArray(
                      _ReactEditor$toDOMPoi,
                      2,
                    ),
                  ...
                  if (
                    ...
                    if (
                      ...
                      ((_lastText$textContent = lastText.textContent) ===
                        null || _lastText$textContent === void 0
                        ? void 0
              ...
              if (
                ...
                var _event$getTargetRange = event.getTargetRanges(),
                  _event$getTargetRange2 = _slicedToArray(
                    _event$getTargetRange,
                    1,
                  ),
                ...
                case "insertText": {
                  ...
                  if (
                    (data === null || data === void 0
                      ? void 0
                      : data.constructor.name) === "DataTransfer"
                  ) {
              ...
              var toRestore =
                (_EDITOR_TO_USER_SELEC =
                  EDITOR_TO_USER_SELECTION.get(editor)) === null ||
                _EDITOR_TO_USER_SELEC === void 0
                  ? void 0
                  : _EDITOR_TO_USER_SELEC.unref();
              ...
              if (
                toRestore &&
                (!editor.selection ||
                  !Range.equals(editor.selection, toRestore))
              ) {
        ...
        var callbackRef = React.useCallback(
          function (node) {
            if (node == null) {
              ...
              NODE_TO_ELEMENT["delete"](editor);
              // ПРОПАТЧЕНО: http://online.saby.ru/opendoc.html?guid=f9aff325-64c6-486b-bb30-6afc29c7a4f8&client=3
              // Реальный анмаунт Editable — любой ещё не сработавший ретрай
              // focus() для этого editor уже гарантированно бесполезен.
              cancelPendingFocusRetry(editor);
          ...
          if (
            ...
            decorations.push(
              _objectSpread(
                _objectSpread(
                  _objectSpread(
                    _defineProperty$2({}, MARK_PLACEHOLDER_SYMBOL, true),
                    unset,
                  ),
                  marks,
                ),
                      ...
                      onDrop: React.useCallback(
                        function (event) {
                          if (
                            ...
                            var range = ReactEditor.findEventRange(
                              editor,
                              event,
                            );
                      ...
                      onKeyDown: React.useCallback(
                        function (event) {
                          if (
                            ...
                            var isRTL =
                              direction_1(Node.string(element)) === "rtl";
                            ...
                            if (hotkeys.isUndo(nativeEvent)) {
                              ...
                              if (
                                typeof _maybeHistoryEditor.undo === "function"
                              ) {
     ...
     */
    var defaultScrollSelectionIntoView =
      function defaultScrollSelectionIntoView(
        ...
        var isBackward =
          !!editor.selection && Range.isBackward(editor.selection);
    ...
    var handleNativeHistoryEvents = function handleNativeHistoryEvents(
      editor,
      event,
    ) {
      var maybeHistoryEditor = editor;
          ...
          if (!slate.Editor.isEditor(editor)) {
            throw new Error(
              "[Slate] editor is invalid! You passed: ".concat(
                slate.Scrubber.stringify(editor),
              ),
            );
          }
